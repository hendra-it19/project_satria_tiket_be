...
@section('pages')
    <div class="row">
        <div class="col-12 my-3">
            <p><a href="{{ route('tickets.index') }}">Tiket > </a> Ubah Data</p>
            <h3 class="text-primary">Ubah Data Tiket</h3>
        </div>

        <div class="col-12">
            <form action="{{ route('tickets.update', $ticket->id) }}" method="post" class="row">
                @csrf
                @method('put')
                ...
                <div class="mb-3 col-12 col-md-5">
                    ...
                    @enderror">
                        ...
                        @foreach ($kapal as $row)
                            <option value="{{ $row->id }}" @if (old('kapal', $ticket->ship_id) == $row->id) selected @endif>
                                {{ $row->nama_kapal }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3 col-12 col-md-5">
                    ...
                    @enderror">
                        ...
                        @foreach ($tujuan as $row => $value)
                            <option value="{{ $value }}" @if (old('tujuan', $ticket->tujuan) == $value) selected @endif>
                                {{ $value }}</option>
                        @endforeach
                    </select>
                    @error('tujuan')
                ...
                <div class="mb-3 col-12 col-md-5">
                    ...
                    <input type="datetime-local" id="keberangkatan" name="keberangkatan"
                        ...
                        is-invalid
                    @enderror"
                        value="{{ old('keberangkatan', $ticket->keberangkatan) }}">
                    @error('keberangkatan')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
